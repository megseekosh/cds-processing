...
wd_typesc <- wd_typesb %>% 
          filter(word!='emily' & word!='emma' & word!='erin' & word!='eve' & word!='francis' & word!='fred'& word!='collin'
...
token_comp2 <- token_comp$contrasts %>%
  ...
  rename(`Timepoints (months)`= contrast,
         ...
         `t-statistic`=t.ratio,
         `p-value`=p.value) %>%
  mutate(`Timepoints (months)`=recode(`Timepoints (months)`,"1 - 2"="7 - 10/11",
                           "1 - 3"="7 - 18",
                           "2 - 3"="10/11 - 18",
                           "1 - 4"="7 - 24",
                           "2 - 4"="10/11 - 24",
...
# types and low-freq words  
type_freq_10mo <- lm(low_freq_wds~Types, data=slope_data3)

# acoustic measures
freq_dur <- coartic_type_df %>%
            filter(merged_tpt=='2') %>%
            lm(Phone_duration_centered*1000~
               SFreq_centered,
               data=.)

coartic_type_df$Phone_duration_ms_centered <- coartic_type_df$Phone_duration_centered*1000 
dur_coartic <- coartic_type_df %>%
            filter(merged_tpt=='2') %>%
            lm(euc_dist_centered~
               Phone_duration_ms_centered,
               data=.)
```
...
stargazer(freq_10mo, 
            ...
            style = "all",
            order=c(4,1,2,3),
            covariate.labels = c("Intercept","Word frequency", "Gender:male", "Mat. Ed."))
```
